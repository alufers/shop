    ...
    <div>
        <v-dialog v-model="dialog" max-width="500px">
            <v-btn slot="activator" color="primary" dark class="mb-2">
                <template v-if="!parentID">New category</template>
                <template v-else>New subcategory</template>
            </v-btn>
            <v-card>
                <v-card-title>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="primary darken-1" @click.native="save" :disabled="category.Name.trim() === ''">Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
export default {
  ...
  methods: {
    ...
    async save() {
      let url;
      if (this.parentID) {
        url = encodeParams`/api/shop/categories/${this.parentID}/children`;
      } else {
        url = "/api/shop/categories";
      }
      try {
        await api.post(url, this.category);
        this.snackbarColor = "primary";
        this.snackbar = `Category ${this.category.Name} added.`;
        this.close();
      } catch (e) {
